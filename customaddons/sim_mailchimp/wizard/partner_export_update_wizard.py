...
class ParterExportMailchimp(models.TransientModel):
    ...
    odoo_list_ids = fields.Many2many('mailchimp.lists', string='MailChimp Lists',
                                     domain=[('odoo_list_id', '!=', False)])
    ...
    def get_mailing_contact_id(self, partner_id, force_create=False):
        ...
        prepared_vals = {'name': partner_id.name, 'email': partner_id.email,
                         'tag_ids': [(6, 0, partner_id.category_id.ids)], 'country_id': partner_id.country_id.id}
        if not contact_id:
    ...
    def action_export_partner_mailchimp(self):
        ...
        for partner_id in partner_ids:
            for odoo_list_id in self.odoo_list_ids:
                ...
                if contact_id:
                    contact_id = mailing_contact_obj.browse(contact_id)
                    if odoo_list_id.id not in contact_id.subscription_list_ids.mapped('list_id').mapped(
