    ...
    def mailchimp_api(self, **kwargs):
        ...
        if kwargs.get('data[merges][EMAIL]', False):
            ...
            contact_id = contact_obj.search(
                ['|', ('email', '=', email_address), ('subscription_list_ids.mailchimp_id', '=', mailchimp_id)],
                limit=1)
            ...
            if kwargs.get('data[action]', '') != 'delete':
                ...
                prepared_vals_for_create_partner = mailchimp_list_id._prepare_vals_for_to_create_partner(
                    merge_field_dict)
                ...
                if not contact_id:
                    ...
                    contact_id = contact_id.create({'name': name, 'email': email_address,
                                                    'country_id': prepared_vals_for_create_partner.get('country_id',
                                                                                                       False) or False})
                if contact_id and kwargs.get('data[action]', '') != 'delete':
                    ...
                    vals = {'list_id': mailchimp_list_id.odoo_list_id.id, 'contact_id': contact_id.id,
                            'mailchimp_id': mailchimp_id, 'md5_email': md5_email}
