...
class MailChimpMergeFields(models.Model):
    ...
    type = fields.Selection(
        [('text', 'Text'), ('number', 'Number'), ('address', 'Address'), ('phone', 'Phone'), ('date', 'Date'),
         ('radio', 'Radio'), ('dropdown', 'Dropdown'),
    ...
    display_order = fields.Char("Display Order",
                                help="The order that the merge field displays on the list signup form.")
    list_id = fields.Many2one("mailchimp.lists", string="Associated MailChimp List", ondelete='cascade', required=True,
                              copy=False)
    field_id = fields.Many2one('ir.model.fields', string='Odoo Field',
                               help="""Odoo will fill value of selected field while contact is going to export or update""",
