...
class MailchimpQueueProcess(models.Model):
    ...
    def do_fetch(self):
        ...
        if self.state != 'in_queue':
            return False
        response = self.account_id._send_request(self.req_url, safe_eval(self.req_data),
                                                 params=safe_eval(self.req_param))
    ...
    def process_queue_response_data(self):
        for pending_record in self.search([('state', 'in', ['in_queue', 'fetched']), ('account_id', '!=', False)],
                                          order='id'):
            # for pending_record in self.search([('state', 'in', ['in_queue', 'fetched']), ('account_id', '!=', False)], order='id'):
            ...
            if pending_record.pending_res_data:
                ...
                if pending_record.operation == 'campaign_activity_report' and pending_record.campaign_id:
                    pending_record.campaign_id.process_report_from_stored_response(pending_record)
