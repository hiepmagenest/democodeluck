...
class massMailingContact(models.Model):
    ...
    def _prepare_vals_for_merge_fields(self, mailchimp_list_id):
        ...
        for custom_field in mailchimp_list_id.merge_field_ids:
            if custom_field.type == 'address' and partner_id:
                ...
                value = getattr(partner_id or self, custom_field.field_id.name) if custom_field.field_id and hasattr(
    ...
    def update_tag_on_mailchimp(self, response, mailchimp_list_id, md5_email):
        ...
        tag_res = mailchimp_list_id.account_id._send_request(
            'lists/%s/members/%s/tags' % (mailchimp_list_id.list_id, md5_email), tag_vals, method='POST')
        return tag_res

    def fetch_specific_member_data(self, mailchimp_list_id, md5_email):
        member_response = mailchimp_list_id.account_id._send_request(
            'lists/%s/members/%s' % (mailchimp_list_id.list_id, md5_email), {}, method='GET')
