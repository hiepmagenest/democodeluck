...
class MassMailing(models.Model):
    ...
    def _compute_pending_queue_count(self):
        ...
        for record in self:
            record.pending_queue_count = queue_obj.search_count(
                [('campaign_id', '=', record.id), ('state', 'in', ['in_queue', 'fetched'])])
    ...
    mailchimp_body_html = fields.Html(string='Mailchimp Body', sanitize_attributes=False,
                                      related='mailchimp_template_id.body_html', store=True)
    ...
    def action_view_clicked(self):
        ...
        return {
            ...
            'view_mode': 'tree',
            'view_id': self.env.ref('sim_mailchimp.mailchimp_link_tracker_view_tree').id,
    ...
    def _create_or_update_campaign_statsticts(self):
        self._cr.execute("""
                    ...
                    FROM ( 
                        %s 
                        %s 
                        WHERE mcrel.list_id = mmlr.mailing_list_id
                          AND email IN
                            (SELECT email
                             FROM mailing_trace
                             WHERE mass_mailing_id = %d)
                    ) AS mc_report                
                    WHERE mailing_trace.email = mc_report.email
                    AND mailing_trace.mass_mailing_id = %d""" % (
            self._select_clause(), self._from_clause(), self.id, self.id))
        self._cr.execute("""
    ...
    def process_report_from_stored_response(self, pending_record):
        email_lists = safe_eval(pending_record.pending_res_data)
        account = self.mailchimp_template_id.account_id
        ...
        if email_lists:
            ...
            self._cr.execute(
                'CREATE TABLE tmp_activity_report (email_address character varying, status character varying, mass_mailing_id integer, opened timestamp without time zone, clicked timestamp without time zone, bounced timestamp without time zone)')
            ...
            self._cr.commit()
        self._create_or_update_campaign_statsticts()
        _logger.info("## MAILCHIMP CAMPAIGN ACTIVITY REPORT IMPORT For %s " % (pending_record.name))
        return True
    ...
    def create_or_update_campaigns(self, values_dict, account=False):
        ...
        try:
            ...
            author_id, email_from = self.env['mail.thread']._message_compute_author(None,
                                                                                    settings_dict.get('from_name'),
    ...
    def _prepare_vals_for_export(self):
        ...
        if self.mailchimp_segment_id.mailchimp_id:
            prepared_vals['recipients'].update(
                {'segment_opts': {'saved_segment_id': int(self.mailchimp_segment_id.mailchimp_id)}})
