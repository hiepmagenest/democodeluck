...
NOT_REQUIRED_ON_UPDATE = ['color', 'list_id', 'web_id', 'from_name', 'from_email', 'subject', 'partner_id',
                          'account_id',
                          'date_created', 'list_rating', 'auto_export_filter', 'auto_export_contact', 'segment_ids',
                          'merge_field_ids', 'member_since_last_changed',
                          ...
                          'is_update_required', 'contact_ids', 'subscription_contact_ids',
                          'mailchimp_list_id']
...
class MailChimpLists(models.Model):
    ...
    auto_export_filter = fields.Char('Apply Filter', help="This condition according to filter records",
                                     default='[["email","!=",False]]')
        ...
        while member_data:
            for member in member_data[:100]:
                ...
                contact_id = mailing_contact_obj.search(['|', ('email', '=', member.get('email_address')),
                                                         ('subscription_list_ids.mailchimp_id', '=', mailchimp_id)],
                                                        limit=1)
                ...
                if create_vals.get('FNAME', False) or create_vals.get('LNAME', False):
                    name = "%s %s" % (create_vals.get('FNAME', False) and create_vals.pop('FNAME') or '',
                                      create_vals.get('LNAME', False) and create_vals.pop('LNAME') or '')
                ...
                if contact_id:
                    ...
                    vals.update({'opt_out': True}) if status in ['unsubscribed', 'cleaned'] else vals.update(
                        {'opt_out': False})
            ...
            self._cr.commit()
            _logger.info(
                "## MAILCHIMP IMPORT For %s : CURRENTLY PROCESSED RECORD COUNT : %s" % (pending_record.name, count))
        return True
    ...
    def fetch_members(self):
        ...
        while True:
            prepared_vals.update({'count': count, 'offset': offset,
                                  'fields': 'members.email_address,members.merge_fields,members.tags,members.web_id,members.status'})
            ...
            _logger.info("{job} : Job Queue Created to Fetch MailChimp Audience for '{list}'".format(job=job_queue.name,
                                                                                                     list=self.name))
            self._cr.commit()
    ...
    def auto_export_member_to_list(self):
        for record in self:
            if record.auto_export_filter:
                prepare_domain = safe_eval(record.auto_export_filter)
                models_records = self.env['res.partner'].search(prepare_domain)
                remainig_to_export = models_records.filtered(
                    lambda x: record not in x.subscription_list_ids.mapped('mailchimp_list_id'))
                remainig_to_export.action_export_partner_mailchimp(record)
        return True

    def get_auto_export_member_action(self):
        action = self.env.ref('base.ir_cron_act').read()[0]
        cron = self.env.ref('sim_mailchimp.auto_export_member_to_list')
        if cron:
