...
class MailChimpTemplates(models.Model):
    ...
    def export_update_templates_mailchimp(self, account=False):
        for record in self:
            ...
            prepared_vals = {'name': record.name, 'html': body_html, 'type': record.type,
                             'drag_and_drop': record.drag_and_drop, 'responsive': record.responsive}
            if record.template_id:
                # return True
                response = record.account_id._send_request('templates/%s' % (record.template_id), prepared_vals,
                                                           method='PATCH')
            else:
                response = record.account_id._send_request('templates', prepared_vals, method='POST')
            # account.covert_date(values_dict.get(item))
            date_created = response.get('date_created') and datetime.strptime(response.get('date_created'),
                                                                              "%Y-%m-%dT%H:%M:%S+00:00") or False
            date_edited = response.get('date_edited') and datetime.strptime(response.get('date_edited'),
                                                                            "%Y-%m-%dT%H:%M:%S+00:00") or False
            record.write({
