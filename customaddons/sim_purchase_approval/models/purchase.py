...
class PurchaseOrder(models.Model):
    ...
    def onchange_partner_id(self):
        ...
        if self.partner_id:
            rep_ids = {'domain': {'supplier_rep_ids': [('id', 'in', self.partner_id.child_ids.ids)]}}
            res.update(rep_ids)
            return res
    ...
    def action_button_approve(self):
        for rec in self:
            template_id = self.env.ref('sim_purchase_approval.email_template_rfq_approved')
    ...
    def action_send_for_approval(self):
        for record in self:
            template_id = self.env.ref('sim_purchase_approval.email_template_rfq_approval_request')
            ...
            if record.purchase_order_approval_rule_ids:
                ...
                record.message_post(body=msg, subtype_xnlid='mail.mt_comment')
...
class PurchaseRejectionReason(models.TransientModel):
    ...
    def button_reject(self):
        template_id = self.env.ref('sim_purchase_approval.email_template_rfq_rejected')
