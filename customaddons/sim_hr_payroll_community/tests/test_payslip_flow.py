...
from odoo.addons.sim_hr_payroll_community.tests.common import TestPayslipBase
...
class TestPayslipFlow(TestPayslipBase):
    ...
    def test_00_payslip_flow(self):
        ...
        context = {
            ...
            "active_ids": [self.ref("sim_hr_payroll_community.menu_department_tree")],
            "active_id": self.ref("sim_hr_payroll_community.menu_department_tree")
        }
        ...
        data, data_format = self.env.ref('sim_hr_payroll_community.action_report_payslip').render(richard_payslip.ids)

        # I print the payslip details report
        data, data_format = self.env.ref('sim_hr_payroll_community.payslip_details_report').render(richard_payslip.ids)

        # I print the contribution register report
        context = {'model': 'hr.contribution.register', 'active_ids': [self.ref('sim_hr_payroll_community.hr_houserent_register')]}
        test_reports.try_report_action(self.env.cr, self.env.uid, 'action_payslip_lines_contribution_register', context=context, our_module='sim_hr_payroll_community')
